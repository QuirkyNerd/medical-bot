...
function EHR_KEY(): string {
  return `medos_ehr_${userPrefix()}`;
}
...
// ============================================================
// Storage keys — SCOPED PER USER to prevent cross-user leakage
// ============================================================

// Set/clear the active user ID when the user logs in/out.
...
function userPrefix(): string {
  // Falls back to 'guest' only for unauthenticated local use
  return _activeUserId ? `user_${_activeUserId}` : 'guest';
}

function KEYS() {
  const p = userPrefix();
  return {
    medications:    `medos_${p}_medications`,
    medicationLogs: `medos_${p}_medication_logs`,
    appointments:   `medos_${p}_appointments`,
    vitals:         `medos_${p}_vitals`,
    records:        `medos_${p}_records`,
    history:        `medos_${p}_history`,
    medicines:      `medos_${p}_medicines`,
    contacts:       `medos_${p}_contacts`,
  } as const;
}

/** Wipe all localStorage data for the current user on logout. */
export function clearUserData(): void {
  if (typeof localStorage === 'undefined') return;
  const keys = KEYS();
  Object.values(keys).forEach((k) => localStorage.removeItem(k));
  // Also clear the EHR profile
  const ehrKey = `medos_ehr_${userPrefix()}`;
  localStorage.removeItem(ehrKey);
  // Reset user id
  _activeUserId = null;
}
