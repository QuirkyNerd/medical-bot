...
export function HealthDashboard({
  ...
  const medsTotal = todayMedSlots.length + dbSchedules.length;

  const handleExportPDF = async () => {
    try {
      const t = localStorage.getItem("medos_auth_token");
      if (!t) return onExport(); // fallback to JSON if guest
      ...
      a.href = url;
      a.download = `medos-report-${new Date().toISOString().slice(0,10)}.pdf`;
      a.click();
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 sm:p-8 pb-mobile-nav scroll-touch">
      <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {/* Today's medications */}
        {(todayMedSlots.length > 0 || dbSchedules.length > 0) && (
          <DashSection
            title={t("health_todays_meds", language)}
            icon={Pill}
            onMore={() => onNavigate("schedule")}
          >
            <div className="space-y-2">
              {dbSchedules.map((s) => (
                ...
                >
                  <button
                    onClick={() => onUpdateDbScheduleStatus?.(s.id, s.status === "done" ? "pending" : "done")}
                  ...
                  >
                    {s.status === "done" && <Check size={14} strokeWidth={3} />}
                  </button>
                  <div className="flex-1 min-w-0">
                    <span className={`font-semibold text-sm ${s.status === "done" ? "text-ink-muted line-through" : "text-ink-base"}`}>
                      {s.medication_name}
                    </span>
